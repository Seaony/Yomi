...
struct AiAndUsageTests {
    ...
    func pageCapAndMissingCursorsMarkSpendPartial() async throws {
        ...
        AiAndTestURLProtocol.handler = { _ in
            count += 1
            return (200, Data("{\"data\":[{\"cost\":\"1\",\"currency\":\"usd\"}],\"has_more\":true,\"next_after\":\"cursor-\(count)\",\"next_after_id\":\"id-\(count)\"}".utf8))
        }
        let capped = try await AiAndUsageFetcher.fetch(apiKey: "key", session: Self.session(), environment: [:])
        #expect(count == AiAndUsageFetcher.maxPages)
...
private final class AiAndTestURLProtocol: URLProtocol, @unchecked Sendable {
